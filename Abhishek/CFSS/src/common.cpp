...
void Registration::setpasswd(char *pass)
{
...
{	
	...
	{
		int a= line.find(",");
		mnumber= line.substr(0,a); // encrypted mobile
		pww=line.substr(a+1); // encrypted password
		
		usr=ed.decryption(mnumber); // decrypted mobile
		pass=ed.decryption(pww); // decrypted password
...
string encdec::encryption(string s)
{
	int l = s.length();
	int b = ceil(sqrt(l));
	int a = floor(sqrt(l));
...
string encdec::decryption(string s){
	int l = s.length();
	int b = ceil(sqrt(l));
	int a = floor(sqrt(l));
	string decrypted;
	...
	int c=decrypted.find(" ");
	decrypted=decrypted.substr(0,c);
	return decrypted;
}